...
**`SpineDetectorOBB-aug`** (yolo11n-obb, 4TU, deg90, 20 ep) — strong across
all rotation buckets. Full yolo26s `4tu-ieee` run must beat these numbers on
**every** bucket, not just pooled mAP.
...
| `SpineDetectorOBB-aug.mlpackage` | yolo11n-obb | 4TU | 1024 | 90 | 20 | 100% | 0.973 / 0.966 / 0.976 / 0.968 / 0.969 | **Current best.** Run dir gone; Core ML + this row remain. |
...
Add a row (and update **Current best** if it wins). Keep the sidecar next to
the `.mlpackage`; don’t paste full provenance here.
